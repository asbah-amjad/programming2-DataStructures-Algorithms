    ...
    while(true) {
        ...
        if(command == "U" or command == "u") {
            ...
            // TODO: Add functionality here
                ...
                std::string student_no = parts.at(1);
                std::map< std::string, Student* >::iterator itr = student_numbers.find(student_no);
                
                if(itr == student_numbers.end()){
                    ...
                    getline(std::cin, phone_no);
                    std::cout << std::endl;

                    if(is_valid_phone_number(phone_no)){
                        //phone number update
                        Student s = *(itr->second);
                        s.phone_number = phone_no;

                        //open file in write mode
                        std::ofstream outfile;
                        outfile.open(file_name);

                        std::map< std::string, Student* >::iterator u_it = user_ids.find(student_no);
                        if(u_it != user_ids.end()){
                            Student u = *(itr->second);
                            u.phone_number = phone_no;
                            outfile << u.phone_number;
                        }
                        outfile.close();
                    }
                }
